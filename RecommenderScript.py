...
def runSingleItem(corr, movie_ids_list, movie_index):
    ...
    index = movie_ids_list.index(movie_index)
    corr_coffey_hands  = list(corr[index])
    k = 10
...
def runFullItem(corr, movie_ids_list, movie_indices):
    movieIds = []
    corrs = []
    for movie_index in movie_indices:
        selected_movie_ids, selected_corr = runSingleItem(corr, movie_ids_list, movie_index)
